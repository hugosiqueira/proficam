...
<header class="dashboard_header">
    <div class="dashboard_header_title">
        <h1 class="icon-bullhorn">Avisos</h1>
    ...
    <div class="dashboard_header_search">
        <a title="Nova Hellobar" href="dashboard.php?wc=hello/create" class="btn btn_green icon-plus">Novo Aviso!</a>
    </div>
...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()):
        ...
        echo Erro("<span class='al_center icon-notification'>Ainda não existem avisos cadastrados {$Admin['user_name']}. Comece agora mesmo criando seu primeiro aviso!</span>", E_USER_NOTICE);
    else:
        foreach ($Read->getResult() as $HELLO):
            extract($HELLO);

            $hello_cta = (!empty($hello_cta) ? $hello_cta : "Aqui um call to action!");
            $hello_title = (!empty($hello_title) ? $hello_title : "Você precisa editar esse aviso!");
            ...
            echo "<article class='box box25 page_single hello_single' id='{$hello_id}'>
                ...
                <div class='page_single_action'>
                    <a title='Editar Aviso' href='dashboard.php?wc=hello/create&id={$hello_id}' class='post_single_center icon-pencil btn btn_blue'>Editar</a>
                    <span rel='hello_single' class='j_delete_action icon-cancel-circle btn btn_red' id='{$hello_id}'>Excluir</span>
                    <span rel='hello_single' callback='Custom' callback_action='hellobar_delete' class='j_delete_action_confirm icon-warning btn btn_yellow' style='display: none' id='{$hello_id}'>Deletar Aviso?</span>
                </div>
            </article>";
        endforeach;
