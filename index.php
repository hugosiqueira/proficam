    ...
    <html lang="pt-br" itemscope itemtype="https://schema.org/<?= $SEO->getSchema(); ?>">
        ...
        <meta name="mit" content="2017-12-04T19:31:24-02:00+9549">
          ...
          rel="stylesheet">
            ...
            if (file_exists('themes/' . THEME . '/style_aggregate.css')):
                $css_files = file('themes/' . THEME . '/style_aggregate.css');
                foreach ($css_files as $css_line):
                    $css_line = str_replace (array("\r\n", "\n", "\r"), ' ', $css_line);
                
                    if (preg_match('!/\*.*?\*/!s', $css_line)):  // Excluir comentários        
                    elseif (preg_match('/(\/\*)|(\*.*)|(\*\/)/', $css_line)): // Excluir comentários        
                    elseif (substr($css_line,0,1)==='/'):
                        echo '            <link rel="stylesheet" type="text/css" href="' . BASE . '/_cdn' . $css_line . '"/>' . "\r\n";
                    else:
                        echo '            ' . $css_line . "\r\n";
                    endif;
                endforeach;
