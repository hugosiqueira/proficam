...
if ($_SERVER['HTTP_HOST'] == 'localhost'):
    ...
    define('SIS_DB_DBSA', 'workcontrol'); //Nome  do banco de dados no localhost
else:
    define('SIS_DB_HOST', 'localhost'); //Link do banco de dados no servidor
    define('SIS_DB_USER', 'agen_proficam'); //Usuário do banco de dados no servidor
    define('SIS_DB_PASS', '8av1yOGafcIdzs5k'); //Senha  do banco de dados no servidor
    define('SIS_DB_DBSA', 'agen_proficam'); //Nome  do banco de dados no servidor
endif;
/*
 * CACHE E CONFIG
 */
...
define('DB_HELLO', 'ws_hellobar'); //Controle de code de WC


/*
 * ITV
 */
define ('DB_COURSE', 'itv_course');
define ('DB_COURSE_DEGREE', 'itv_course_degree');
define ('DB_AREA_CONCENTRACAO', 'itv_area_concentracao');
define ('DB_RESEARCH', 'itv_research');
define ('DB_TEACHERS', 'itv_teachers');
define ('DB_STUDENTS', 'itv_students');
define ('DB_STUDENTS_STATUS', 'itv_students_status');
define ('DB_STUDENTS_CLASS', 'itv_students_class');
define ('DB_TESTIMONIALS', 'itv_students_testemonials');
define ('DB_FAQ', 'itv_faq');
define ('DB_FAQ_CATEGORY', 'itv_faq_category');
define ('DB_GALLERY', 'itv_gallery');
...
function getStudentsCategory($category){
    $StudentsCategory = [
        1 => 'Matriculado',
        2 => 'Egresso',
        3 => 'Desligado',
    ...
    if (!empty($category)):
        return $StudentsCategory[$category];
    else:
        return $StudentsCategory;
    endif;
}

function getStudentsClass($class){
    $Read = new Read;
    $Read->ExeRead( DB_STUDENTS_CLASS, "WHERE students_class_id = {$class}");
    $class = $Read->getResult()[0];
    return $class['students_class_name'];
...
function getTitle($title){
    $Read = new Read;
    $Read->ExeRead( DB_COURSE_DEGREE, "WHERE course_degree_id = {$title}");
    $class = $Read->getResult()[0];
